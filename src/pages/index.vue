...
import { PublicKey, Account, Transaction } from '@solana/web3.js'
...
export default Vue.extend({
  ...
  methods: {
      ...
      if(found){
        ...
        this.prices.forEach((item:PriceInfo)=>{
          ...
          if(parseFloat(item.midCoinStableAmount) > parseFloat(this.best_midCoinStableAmount))
          {
            ...
            // this.best_midCoinWormholeAmount = this.fromCoinAmount
          }
        })
      }
      else{
    ...
    async placeOrder() {
      ...
      let amountNew = ''
      if (this.isWrap) {
        const {transaction, signers} = await wrap(
          this.$axios,
          // this.$web3,
          // @ts-ignore
          this.$wallet,
        ...
        signersAll = [...signersAll, ... signers]

        amountNew = this.fromCoinAmount
      } 
      else if (this.best_endpoint === 'Raydium Pool' && this.ammId) {

        const poolInfo = Object.values(this.$accessor.liquidity.infos).find((p: any) => p.ammId === this.ammId)
        const {transaction, signers} =  await raydiumSwap(
          this.$web3,
          // @ts-ignore
          this.$wallet,
          poolInfo,
        ...
        amountNew = this.best_midCoinStableWithSlippage
      } else if(this.best_endpoint === 'Serum Dex') {
        const {transaction, signers, amountEstimated} =  await serumPlace(
          this.$web3,
          // @ts-ignore
          this.$wallet,
          this.market,
        ...
        amountNew = '' + amountEstimated 
      }

      const {transaction, signers} =  await stableSwap(
        this.$web3,
        // @ts-ignore
        this.$wallet,
        STABLE_POOLS.USDT,
      ...
      try{
        const txid = await sendTransaction(this.$web3, this.$wallet, transactionAll, signersAll)
        this.$notify.info({
          key,
        ...
        this.$accessor.transaction.sub({ txid, description })
        const amountTransfer = '' + (Math.round(parseFloat(amountNew) * 996) / 1000)
        this.doWormholeTransfer(amountTransfer)
      }
      catch(error){
        this.$notify.error({
          key,
