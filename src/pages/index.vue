...
// swap as raydiumSwap, 
// wrap, 
checkUnsettledInfo, settleFund } from '@/utils/swap'
import {
  // stableSwap, 
  STABLE_POOLS, 
  MERCURIAL_POOLS
  ...
  methods: {
    ...
    findMarket() {
      if (this.fromCoin && this.midCoinStable && this.liquidity.initialized) {
        ...
        let feeTokenMint = this.fromCoin?.mintAddress
        if (feeTokenMint === NATIVE_SOL.mintAddress) feeTokenMint = TOKENS.WSOL.mintAddress
        getOneFilteredTokenAccountsByOwner(this.$web3, new PublicKey(FEE_OWNER), new PublicKey(feeTokenMint as string)).then((account)=>{
    ...
    placeOrder() {
      ...
      if (!this.feeTokenAccount || this.needCreateTokens() || this.needWrapSol()) {
